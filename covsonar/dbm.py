...
class sonarDBManager:
    ...
    def connect(self):
        ...
        try:
            if self.dbfile is not None:
                ...
                logging.warning("DB URL did not given, fall back to .env file")
                db_user = DB_USER
                db_pass = DB_PASS
                db_url = DB_URL
                db_port = DB_PORT
    ...
    def insert_sample(self, sample_name, seqhash):
        ...
        sql = "REPLACE INTO sample (name, seqhash, datahash) VALUES(?, ?, ?);"
    ...
    def match(  # noqa: 901
        ...
        *profiles,
        ...
        if format == "csv" or format == "tsv":
            ...
            _2_final_sql = (
                " SELECT name AS `sample.name`, nt_profile._profile AS NUC_PROFILE, aa_profile._profile AS AA_PROFILE \
                    FROM ( SELECT  `sample.id`, group_concat("
                + m
        ...
        print(sql, property_vals + profile_vals)
        self.cursor.execute(sql, property_vals + profile_vals)
        return self.cursor
