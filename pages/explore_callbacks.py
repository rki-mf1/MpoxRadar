...
def get_explore_callbacks(
    ...
    def actualize_mutation_filter(
        ...
        if dash.ctx.triggered_id == "select_x_frequent_mut_0":
            ...
            variantView_cds_ref_gene = variantView_cds_ref[
                variantView_cds_ref["element.symbol"
                ].isin(gene_value)]
            mut_options = get_frequency_sorted_mutation_by_filters(variantView_cds_ref_gene,
                                                                   propertyView_seq_country,
                                                                   color_dict)
            max_select = len(mut_options)
    ...
    def actualize_gene_filters(reference_value, select_all_genes, gene_options):
        ...
        if dash.ctx.triggered_id == "select_all_genes_0":
            if len(select_all_genes) == 1:
                gene_value = [i['value'] for i in get_all_genes_per_reference(df_dict["variantView"],
                                                                              reference_value,
                                                                              color_dict)]
    ...
    def update_slider_interval(drag_value, interval, n_intervals, slider_value):
        ...
        if dash.ctx.triggered_id in ["selected_interval", "date_slider"]:
            ...
            if len(drag_value) == 2:
                ...
                if DateSlider.get_date_x_days_before(
                    ...
                    new_first_date = DateSlider.unix_time_millis(
                        ...
                        (DateSlider.unix_to_date(second_date),
                         interval)
                    )
                else:
    ...
    def update_table_filter(mutation_list, reference_id, gene_list, seqtech_list, interval, dates, countries):
        date_list = date_slider.get_all_dates_in_interval(dates, interval)
        table_df = table_filter.get_filtered_table(mutation_list,
                                                   seqtech_list,
                                                   reference_id,
                                                   date_list,
                                                   gene_list,
