...
class DfsAndDetailPlot(object):
    ...
    def filter_df(self, mutations, reference_id, seq_tech_list, dates, countries, genes):
...
class WorldMap(DfsAndDetailPlot):
    ...
    def get_world_map_df(self, method, mutations, reference_id, seq_tech_list, dates, countries, genes):
        ...
        if countries is None:
            countries = []
        filtered_df = self.filter_df(mutations, reference_id, seq_tech_list, dates, countries, genes)
        if method == 'Frequency':
...
class DateSlider:
    ...
    def get_all_dates(first_date, second_date):
        ...
        date_list = [d for d in [second_date - timedelta(days=x) for x in reversed(range(days))]]
        return date_list
