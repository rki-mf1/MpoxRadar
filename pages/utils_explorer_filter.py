...
def get_frequency_sorted_mutation_by_filters(
    ...
    df_merge = pd.merge(variantView_mut_ref_select, propertyView_seq_tech,
                        how="inner",
                        on="sample.id")[
    ...
    sorted_mutations_dict = [
        {
            "label": html.Span(gene_mut[1],
                               style={"color": color_dict[gene_mut[0]]}),
            "value": gene_mut[1],
        }
        for gene_mut in list(
            zip(
...
def get_mutations_by_filters(variantView_mut_ref_select, propertyView_seq_tech):
    df_merge = pd.merge(variantView_mut_ref_select, propertyView_seq_tech, how="inner", on="sample.id")[
        ["sample.id", "variant.label", "element.symbol"]
    ]
    return df_merge
