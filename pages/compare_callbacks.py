...
from pages.utils_explorer_filter import  get_frequency_sorted_mutation_by_filters, \
    get_all_genes_per_reference, get_all_frequency_sorted_countries, get_all_frequency_sorted_seqtech
...
def get_compare_callbacks(df_dict, variantView_cds, color_dict):
    ...
    def actualize_mutation_filter(compare_button, gene_value_1, gene_value_2, reference_value_1, seqtech_value_1,
                                  country_value_1, reference_value_2, seqtech_value_2, country_value_2,
                                  start_date_1, end_date_1, start_date_2, end_date_2):
        """

        """

        date_list_1 = DateSlider.get_all_dates(datetime.datetime.strptime(start_date_1, "%Y-%m-%d").date(),
                                               datetime.datetime.strptime(end_date_1, "%Y-%m-%d").date())
        variantView_cds_ref_1 = variantView_cds[(variantView_cds['reference.id'] == reference_value_1)]
        ...
        date_list_2 = DateSlider.get_all_dates(datetime.datetime.strptime(start_date_2, "%Y-%m-%d").date(),
        ...
        mut_options_1 = [{'value': mut, 'label': mut} for mut in unique_mutations_value_1]
        unique_mutations_value_2 = list(set(mut_value_2).difference(set(mut_value_1)))
        mut_options_2 = [{'value': mut, 'label': mut} for mut in unique_mutations_value_2]
        ...
        df_all = pd.merge(df_dict['variantView'], df_dict['propertyView'], how="inner", on=["sample.id", "sample.name"])
        table_columns = ['sample.name', 'reference.accession', 'element.symbol', 'variant.label', "COUNTRY", "SEQ_TECH",
                         "GEO_LOCATION", "ISOLATE"]
        table_df_1 = df_all[df_all['variant.label'].isin(unique_mutations_value_1) &
                            (df_all['reference.id'] == reference_value_1) &
                            (df_all["SEQ_TECH"].isin(seqtech_value_1)) &
                            (df_all["COUNTRY"].isin(country_value_1))][table_columns]
        table_df_2 = df_all[df_all['variant.label'].isin(unique_mutations_value_2) &
                            (df_all['reference.id'] == reference_value_2) &
                            (df_all["SEQ_TECH"].isin(seqtech_value_2)) &
                            (df_all["COUNTRY"].isin(country_value_2))][table_columns]
        table_df_3 = df_all[df_all['variant.label'].isin(intersection_mutation) &
                            (df_all['reference.id'].isin([reference_value_1, reference_value_2])) &
                            (df_all["SEQ_TECH"].isin((seqtech_value_1 + seqtech_value_2))) &
                            (df_all["COUNTRY"].isin((country_value_1 + seqtech_value_2)))][table_columns]
        ...
        return mut_options_1, unique_mutations_value_1, text_1, mut_options_2, unique_mutations_value_2, text_2, \
               mut_options_3, intersection_mutation, text_3, table_df_1_records, column_names_1, table_df_2_records, \
               column_names_2, table_df_3_records, column_names_3
    ...
    def actualize_filters_1(reference_value, select_all_seq_techs, select_all_genes, select_all_countries,
                            gene_options, country_options, seq_tech_options, gene_value, country_value,
                            seq_tech_value):
        """
    ...
    def actualize_filters_2(reference_value, select_all_seq_techs, select_all_genes, select_all_countries,
                            gene_options, country_options, seq_tech_options, gene_value, country_value,
                            seq_tech_value):
        """
