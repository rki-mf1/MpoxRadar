...
def get_filtered_samples(
    propertyView_dfs, variantView_dfs, mut_value, gene_dropdown=None
):
    new_samples = set()
...
def create_mutation_dfs_for_comparison(
    ...
    for i, variantView in enumerate(variantView_dfs):
        merged_dfs.append(merge_df(variantView, propertyView_dfs[i]))
...
def create_comparison_tables(
    ...
    variantView_dfs_left = select_variantView_dfs(
        df_dict, complete_partial_radio, reference_value_1, aa_nt_radio
    )
    variantView_dfs_right = select_variantView_dfs(
        df_dict, complete_partial_radio, reference_value_2, aa_nt_radio
    )
    ...
    samples_left_both = get_filtered_samples(
        propertyView_dfs_left, variantView_dfs_left, mut_value_3, gene_dropdown_1
    )
    samples_right_both = get_filtered_samples(
        propertyView_dfs_right, variantView_dfs_right, mut_value_3, gene_dropdown_2
    )
    samples_left = get_filtered_samples(
        propertyView_dfs_left, variantView_dfs_left, mut_value_1, gene_dropdown_1
    )
    samples_right = get_filtered_samples(
        propertyView_dfs_right, variantView_dfs_right, mut_value_2, gene_dropdown_2
    )

    variantView_dfs_left_both = [
        df[df["sample.id"].isin(samples_left_both)] for df in variantView_dfs_left
    ]
    variantView_dfs_right_both = [
        df[df["sample.id"].isin(samples_right_both)] for df in variantView_dfs_right
    ]
    variantView_dfs_left = [
        df[df["sample.id"].isin(samples_left)] for df in variantView_dfs_left
    ]
    variantView_dfs_right = [
        df[df["sample.id"].isin(samples_right)] for df in variantView_dfs_right
    ]
