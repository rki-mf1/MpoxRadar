...
def merge_df(variantView, propertyView, aa_nt_radio):
    if aa_nt_radio == "cds":
        return pd.merge(variantView, propertyView, how="inner", on="sample.id")[
...
def get_filtered_samples(propertyView_dfs, variantView_dfs, mut_value, variant_col):
    ...
    for i, df in enumerate(variantView_dfs):
        ...
        new_samples = new_samples.union(
            set(
                df[df["sample.id"].isin(samples) & df[variant_col].isin(mut_value)][
                    "sample.id"
                ]
            )
        )
...
def create_comparison_tables(
    ...
    propertyView_dfs_left = [
        filter_propertyView(
            df, seqtech_value_1, country_value_1, start_date_1, end_date_1
        )
        for df in propertyView_dfs
    ]
    propertyView_dfs_right = [
        filter_propertyView(
            df, seqtech_value_2, country_value_2, start_date_2, end_date_2
        )
        for df in propertyView_dfs
    ]
    ...
    samples_left_both = get_filtered_samples(
        propertyView_dfs_left, variantView_dfs, mut_value_3, variant_col
    )

    samples_right_both = get_filtered_samples(
        propertyView_dfs_right, variantView_dfs, mut_value_3, variant_col
    )

    samples_both = samples_left_both.union(samples_right_both)

    samples_left = get_filtered_samples(
        propertyView_dfs_left, variantView_dfs, mut_value_1, variant_col
    )
    samples_right = get_filtered_samples(
        propertyView_dfs_right, variantView_dfs, mut_value_2, variant_col
    )

    # count nb seq in both for left and right selection --> used in actualize_overview_table
    variantView_df_both_left = pd.concat(
        [
            df[
                df["sample.id"].isin(samples_left_both)
                & df[variant_col].isin(mut_value_3)
            ][[variant_col, "sample.id"]]
    ...
    variantView_df_both_left = (
        variantView_df_both_left.groupby([variant_col])
        .size()
        .reset_index()
        .rename(columns={0: "freq l"})
    )
    variantView_df_both_right = pd.concat(
        [
            df[
                df["sample.id"].isin(samples_right_both)
                & df[variant_col].isin(mut_value_3)
            ]
            for df in variantView_dfs
        ],
    ...
    variantView_df_both_right = (
        variantView_df_both_right.groupby([variant_col])
        .size()
        .reset_index()
        .rename(columns={0: "freq r"})
    )
    variantView_df_both = pd.merge(
        variantView_df_both_left,
        variantView_df_both_right,
        how="inner",
        on=[variant_col],
    ...
    sorted_indices = (
        (variantView_df_both["freq l"] + variantView_df_both["freq r"])
        .sort_values(ascending=False)
        .index
    )
    variantView_df_both = variantView_df_both.loc[sorted_indices, :].reset_index(
        drop=True
    )

    variantView_dfs_both = [
        df[df["sample.id"].isin(samples_both)] for df in variantView_dfs
    ]
    variantView_dfs_left = [
        df[df["sample.id"].isin(samples_left)] for df in variantView_dfs
    ]
    variantView_dfs_right = [
        df[df["sample.id"].isin(samples_right)] for df in variantView_dfs
    ]
    table_columns = [
        "sample.name",
        "variant.label",
