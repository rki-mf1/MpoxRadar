...
correct_result_dict = {
    'cds':
    {
        "correct_cols": ["sample.name", "AA_PROFILE", "IMPORTED", "COLLECTION_DATE", "RELEASE_DATE",
        ...
        "table_df_records": [
            {'unique left': 'OPG151:L263F', '# left': 1.0, 'shared': 'OPG193:L263F', '# l': 204.0,
             '# r': 25.0, 'unique right': 'OPG193:A233G', '# right': 3},
            {'unique left': float('nan'), '# left': float('nan'), 'shared': float('nan'),
             '# l': float('nan'), '# r': float('nan'), 'unique right': 'OPG113:D723G', '# right': 2}
        ],

        "overview_left": pd.DataFrame([['OPG151:L263F', 1]], columns=['value', 'freq']),
        "overview_right": pd.DataFrame([['OPG193:A233G', 3], ['OPG113:D723G', 2]],
                                       columns=['value', 'freq']),
        "overview_both": pd.DataFrame([['OPG193:L263F', 204, 25]],
                                      columns=['gene:variant', 'freq l', 'freq r']),

    },
    'source':
    {
        "correct_cols": ["sample.name", "NUC_PROFILE", "IMPORTED", "COLLECTION_DATE",
                         "RELEASE_DATE", "ISOLATE", "LENGTH", "SEQ_TECH", "COUNTRY", "GEO_LOCATION",
                         "HOST", "GENOME_COMPLETENESS", "REFERENCE_ACCESSION"],
        ...
        "table_df_records": [
            {'unique left': 'G74360A', '# left': 37, 'shared': float('nan'), '# l': float('nan'),
             '# r': float('nan'), 'unique right': 'del:150586-150602', '# right': 25.0},
            {'unique left': 'C70780T', '# left': 12, 'shared': float('nan'), '# l': float('nan'),
             '# r': float('nan'), 'unique right': float('nan'), '# right': float('nan')},
            {'unique left': 'G8020A', '# left': 2, 'shared': float('nan'), '# l': float('nan'),
             '# r': float('nan'), 'unique right': float('nan'), '# right': float('nan')},
            {'unique left': 'C11343A', '# left': 1, 'shared': float('nan'), '# l': float('nan'),
             '# r': float('nan'), 'unique right': float('nan'), '# right': float('nan')},
            {'unique left': 'G173318A', '# left': 1, 'shared': float('nan'), '# l': float('nan'),
             '# r': float('nan'), 'unique right': float('nan'), '# right': float('nan')}
        ],
        "overview_left": pd.DataFrame([['G74360A', 37],
                                       ...
                                       ['G173318A', 1]],
                                      columns=['value', 'freq']),
        "overview_right": pd.DataFrame([['del:150586-150602', 25]],
                                       columns=['value', 'freq']),
...
class TestCompareTable(unittest.TestCase):
    ...
    def test_create_comparison_tables_cds(self, aa_nt):
        ...
        for mut in table_df_1[mut_col].to_list():
            assert len(set(mut.split(',')).intersection(
                set(correct_result_dict[aa_nt]["mut_value_left"]))) > 0
        for mut in table_df_2[mut_col].to_list():
            assert len(set(mut.split(',')).intersection(
                set(correct_result_dict[aa_nt]["mut_value_right"]))) > 0
        for mut in table_df_3[mut_col].to_list():
            assert len(set(mut.split(',')).intersection(
                set(correct_result_dict[aa_nt]["mut_value_both"]))) > 0
        self.assertListEqual(
            list(table_df_1['SEQ_TECH'].unique()), correct_result_dict[aa_nt]["seq_tech_left"])
        self.assertListEqual(
            list(table_df_2['SEQ_TECH'].unique()), correct_result_dict[aa_nt]["seq_tech_right"])
        self.assertListEqual(
            list(table_df_3['SEQ_TECH'].unique()), correct_result_dict[aa_nt]["seq_tech_both"])
...
class TestOverviewTable(unittest.TestCase):
    ...
    def setUp(self):
        self.column_names_overview = [
            ...
            {'name': 'shared variants of both selections',
             'id': 'shared'},
            {'name': '# seq left', 'id': '# l'},
            {'name': '# seq right', 'id': '# r'},
            {'name': 'unique variants for right selection',
             'id': 'unique right'},
