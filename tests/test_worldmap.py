...
class TestWorldMap(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.variants1 = {2: DbProperties.cds_gene_variants["complete"][2],
                         4: DbProperties.cds_gene_variants["complete"][4]}
        # T58K A233G appears only once
        cls.variants2 = {2:
                         [
                             'OPG197:T22K', 'OPG151:L263F', 'OPG193:L263F', 'OPG016:R84K', 
                             'OPG113:T58K', 'OPG193:A233G'
                         ],
                         4: DbProperties.cds_gene_variants["complete"][4]}
        ...
        cls.interval2 = 30
        cls.dates21 = (DateSlider.unix_time_millis(datetime.date(2022, 6, 28)),
    ...
    def test_get_df_for_frequency_bar(self, reference, interval, completeness):
        for country in self.countries:
            ...
            correct_df = pd.DataFrame(
                DbProperties.correct_rows_frequency_bar[reference][country][interval][completeness],
                columns=columns
            )
    ...
    def test_increase_df(self, reference, interval, completeness):
        columns = [
            'COUNTRY', 'variant.label', 'element.symbol', 'number_sequences', 'COLLECTION_DATE',
            'slope'
        ]
        for country in self.countries:
            detail_plot_instance = DetailPlots(
            ...
            correct_df = pd.DataFrame(
                DbProperties.correct_rows_increase_df[reference][country][interval][completeness],
                columns=columns
            )
            correct_df = correct_df.sort_values(
    ...
    def test_get_scatter_df(self, reference, interval, completeness):
        columns = [
            'COUNTRY', 'COLLECTION_DATE', 'variant.label', 'element.symbol', 'number_sequences',
            'date_numbers'
        ]
        for country in self.countries:
            detail_plot_instance = DetailPlots(
            ...
            correct_df = pd.DataFrame(
                DbProperties.correct_rows_scatter_df[reference][country][interval][completeness],
                columns=columns
            )
            correct_df = correct_df.sort_values(
    ...
    def test_get_world_frequency_df(self, reference, interval, completeness):
        ...
        df, column_of_interest = world_map_instance.get_world_map_df(
            'Frequency'
        )
    ...
    def test_plots_empty_filter_options(self):
        ...
        correct_df_scatter = pd.DataFrame(
            data=[
                    ['Germany', datetime.date(2022, 10, 2), 'no_mutations', 'no_gene', 0, 96]
            ],
            columns=[
                'COUNTRY', 'COLLECTION_DATE', 'variant.label', 'element.symbol', 'number_sequences',
                'date_numbers'
        ...
        assert_frame_equal(df_scatter,
                           correct_df_scatter,
                           check_datetimelike_compat=True,
                           check_dtype=False
                           )

        df_increase = detail_plot_instance.get_increase_df(detail_plot_instance.filtered_dfs)
